...
// Lexical Analyzer
void analyze(char* input, FILE *output) {
    int left = 0, right;
    int len = strlen(input);
    char* lexeme;

    fprintf(output, "--- Lexical Analysis Results ---\n");
    fprintf(output, "%-25s | %s\n", "Lexeme", "Token Type");
    fprintf(output, "----------------------------------------\n");
    ...
    while (left < len) {
        ...
        if (input[left] == '#') {
            ...
            lexeme[right - left] = '\0';
            fprintf(output, "%-25s | PREPROCESSOR_DIRECTIVE\n", lexeme);
            free(lexeme);
            left = right;
            continue;
        }
        ...
        if (input[left] == '"' || input[left] == '\'') {
            ...
            if (right > left) {
                ...
                strncpy(lexeme, input + left, right - left);
                lexeme[right - left] = '\0';
                fprintf(output, "%-25s | STRING_LITERAL\n", lexeme);
                free(lexeme);
            }
        ...
        if (isKeyword(lexeme)) {
            fprintf(output, "%-25s | KEYWORD\n", lexeme);
        } else if (isInteger(lexeme)) {
            fprintf(output, "%-25s | INTEGER\n", lexeme);
        } else if (isIdentifier(lexeme)) {
            fprintf(output, "%-25s | IDENTIFIER\n", lexeme);
        } else {
            fprintf(output, "%-25s | UNKNOWN\n", lexeme);
        }
...
int main() {
    ...
    analyze(inputBuffer, outputFile); // write to file
